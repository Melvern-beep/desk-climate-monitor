...
**Sensor selection.** Originally planned around a DHT22. Switched to SHT31-DIS after finding the SHT4x family is rated 1.08-3.6V, which would need level shifting on a 5V Nano. The SHT3x family runs 2.4-5.5V and connects directly.

**Non-blocking timing.** All timing uses millis() rather than delay(). delay() halts the entire chip, which makes concurrent operations impossible. This project only needs one timed task, but the pattern is required for the follow-up build.
